...
class UserController extends Controller {
    constructor(version) {
        ...
        this.getPicture = [
            this.validator.user.pictureSearch,
            this._findPictureByLanguageAndName
        ];

        this.getNames = [
            this.validator.user.namesSearch,
            this._findNamesByLanguages
        ];
     ...
     * @private
     */
    _findNamesByLanguages(req, res, next) {
        const searchParams = {};
        searchParams.language = req.query.language;
        db.Users.find(searchParams).distinct('name')
            .then(users => {
                res.status(200).send(Format.wrapper(Format.nameList(users)));
            })
            .catch(next);
    }
     ...
     * @private
     */
    _findPictureByLanguageAndName(req, res, next) {
        const searchParams = {};
        searchParams.language = req.query.language;
        searchParams.name = req.query.name;
        db.Users.find(searchParams).distinct('picture')
            .then(users => {
                res.status(200).send(Format.wrapper(Format.pictureItem(users)));
            })
            .catch(next);
    }
    /**
    ...
    /**
     * get List of 'TABLES' but it just language
     * @param req
     * @param res
     * @param next
     * @private
     */
    _getListLanguages(req, res, next) {
        db.Users.find().distinct('language')
            .then(users => {
                res.status(200).send(Format.wrapper(Format.languageList(users)));
            })
            .catch(next);
    }

    /**
     * get List of 'TABLES' but it just name
     * @param req
     * @param res
     * @param next
     * @private
     */
    _getListNames(req, res, next) {
        db.Users.find().distinct('name')
            .then(users => {
                res.status(200).send(Format.wrapper(Format.nameList(users)));
            })
            .catch(next);
